...
export class VideoFeeder {
    ...
    start = () => {
        ...
        try {
            ...
            defaultResourceManager.registerFfmpegProcess();
        }
        catch (err) {
            this.#running = false;
            this.onError?.(err);
        ...
        proc.on("close", (code) => {
            if (this.#proc === proc) {
                this.#proc = null;
                defaultResourceManager.unregisterFfmpegProcess();
            }
    ...
    #handleData = (chunk) => {
        ...
        // Bound pending buffer to at most 10 frames to avoid unbounded memory accumulation
        const maxPendingBytes = this.frameSizeBytes * 10;
        if (this.#pending.length > maxPendingBytes) {
            // Drop oldest frame data
            const overflow = this.#pending.length - maxPendingBytes;
            this.#pending = this.#pending.subarray(overflow);
        }
