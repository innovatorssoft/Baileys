...
export class SignalingBridge {
    ...
    #doSendSignaling = async (peerJid, callId, xmlPayload) => {
        ...
        const routeTo = (signalingTag === "transport" || signalingTag === "mute_v2" || signalingTag === "offer" || signalingTag === "enc_rekey" || signalingTag === "terminate")
    ...
    #doProcessIncomingCall = async (node, voip, activeCallId) => {
        ...
        if (getBinaryNodeChild(voipChild, "enc") || getBinaryNodeChild(voipChild, "destination")) {
        ...
        const offerSignalingMsg = usableNode.tag === "offer" ? {
            payload: b64,
        ...
        if (usableNode.tag === "offer" && this.#incomingOfferListener) {
            try {
                this.#incomingOfferListener(usableNode, routedPeerJid, callIdForRouting, offerSignalingMsg);
            } catch { }
        }
        if (targetVoip) {
        ...
        if (usableNode.tag !== "offer" && usableNode.tag !== "ack") {
    ...
    #maybeDecryptEnc = async (voipNode, peerJid) => {
        ...
        if (destination) {
            ...
            const myLid = this.#sock?.authState?.creds?.me?.lid || "";
            const myPn = this.#sock?.authState?.creds?.me?.id || "";
            const myDevice = myLid.split(":")[1]?.split("@")[0] || myPn.split(":")[1]?.split("@")[0] || "0";

            // Prioritize <to> nodes targeting our device ID or credentials
            const toNodes = destChildren.filter((c) => c && c.tag === "to");
            toNodes.sort((a, b) => {
                const jidA = String(a.attrs?.jid ?? "");
                const jidB = String(b.attrs?.jid ?? "");
                const matchA = (myLid && jidA === myLid) || (myPn && jidA === myPn) || (myDevice !== "0" && jidA.includes(`:${myDevice}@`));
                const matchB = (myLid && jidB === myLid) || (myPn && jidB === myPn) || (myDevice !== "0" && jidB.includes(`:${myDevice}@`));
        ...
        const peerBare = this.#toBareJid(peerJid);
        const peerUser = peerBare.split("@")[0];
        const peerServer = peerBare.split("@")[1] || "s.whatsapp.net";
        const creator = String(voipNode.attrs?.["call-creator"] ?? "").trim();
        const creatorBare = creator ? this.#toBareJid(creator) : "";
        const callerPn = String(voipNode.attrs?.["caller_pn"] ?? "").trim();
        ...
        const candidateSet = new Set([
            ...
            creatorBare,
            callerPn,
            callerPn ? this.#toBareJid(callerPn) : "",
            participant,
            participant ? this.#toBareJid(participant) : ""
        ]);

        const commonDevices = [0, 17, 18, 19, 1, 2, 3];
        if (peerUser) {
            for (const dev of commonDevices) {
                candidateSet.add(dev === 0 ? `${peerUser}@${peerServer}` : `${peerUser}:${dev}@${peerServer}`);
            }
        }
        if (creator && creator !== peerJid) {
            const creatorUser = creatorBare.split("@")[0];
            const creatorServer = creatorBare.split("@")[1] || "s.whatsapp.net";
            for (const dev of commonDevices) {
                candidateSet.add(dev === 0 ? `${creatorUser}@${creatorServer}` : `${creatorUser}:${dev}@${creatorServer}`);
            }
        }
            ...
            for (const jid of candidates) {
                try {
                    ...
                    if (callKey && callKey.length > 0) {
                        ...
                        if (!rootEnc) {
                            const children = Array.isArray(voipNode.content) ? voipNode.content : [];
                            children.push({
                                tag: "enc",
                                attrs: { v: "2", type: "msg", count: "0" },
                                content: rawKeyBuf
                            });
                            voipNode.content = children;
                        }
                        return voipNode;
                    }
                }
