...
export function parseRelayEndpoints(node) {
    ...
    for (const child of children) {
        ...
        if (child.tag === "relay") {
            uuid = child.attrs?.uuid || uuid;
            if (child.attrs?.self_pid) selfPid = parseInt(child.attrs.self_pid, 10);
            if (child.attrs?.peer_pid) peerPid = parseInt(child.attrs.peer_pid, 10);

            const relayChildren = Array.isArray(child.content) ? child.content : [];
...
export class CallManager extends EventEmitter {
    ...
    acceptCall = async (callId, options = {}) => {
        ...
        // Set up WASM engine & relay if not yet attached
        if (!session.engine) {
        ...
        // Pre-prime audio feeder if audio source is provided
        if (session._audioSource && session._audioSource !== "silence" && !session.audioFeeder) {
    ...
    sendAcceptStanza = async (callId, callCreator, toJid, isVideo = false, rawCallKey = null) => {
        ...
        const effectiveKey = rawCallKey || this.calls.get(callId)?._callKey || this.signaling?.getCallKey(callId);
        if (effectiveKey && this.signaling?.encryptCallKey) {
            try {
                const target = callCreator || toJid;
                ...
                if (toJid && toJid !== callCreator) {
                    try {
                        const encrypted = await this.signaling.encryptCallKey(toJid, effectiveKey, 0);
                        encNode = encrypted.encNode;
                        shouldIncludeDeviceIdentity = encrypted.shouldIncludeDeviceIdentity;
                    } catch { }
                }
