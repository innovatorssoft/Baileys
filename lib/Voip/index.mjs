...
export class VoipClient extends EventEmitter {
    ...
    #initCallManager = () => {
        ...
        this.#callManager.on("call_incoming", (session) => {
        ...
        this.#callManager.on("call_terminate_sent", (info) => this.emit("call_terminate_sent", info));
        this.#callManager.on("call_ended", (session, reason) => {
            this.#activeCalls.delete(session.callId);
            this.emit("call_ended", session, reason);
        });
    ...
    call = async (phoneNumber, opts = {}) => {
        ...
        call.on("ended", (reason) => {
            ...
            // The CallManager already emitted (or the peer already signalled) the end —
            // do not put a second terminate on the wire.
            const endAlreadySignalled = Boolean(call._endSignalSent || call._peerInitiatedEnd);
            if (reason !== "remote_end" && reason !== "rejected") {
                try { call.engine?.endCall(0, !endAlreadySignalled); } catch { }
                if (!endAlreadySignalled) {
        ...
        this.#signaling.setSignalingEventListener((tag, reason, callId, peerJid) => {
            ...
            if (peerJid) {
                ...
                if (bare && bare !== selfPn && bare !== selfLid) {
                    const allCalls = [...this.#activeCalls.values(), ...(this.#callManager?.calls.values() || [])];
