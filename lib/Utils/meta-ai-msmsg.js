...
const decryptMsmsgBotMessage = async (messageSecret, messageKey, msMsg) => {
    ...
    const baseKey = Buffer.from((0, crypto_1.hkdf)(toBuffer(messageSecret), KEY_LENGTH, { info: BOT_MESSAGE_INFO }));
    ...
    for (const msgId of msgIdCandidates) {
        ...
        for (const targetJid of targetJidCandidates) {
            ...
            const key = Buffer.from((0, crypto_1.hkdf)(baseKey, KEY_LENGTH, { info }));
