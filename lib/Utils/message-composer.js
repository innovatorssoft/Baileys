...
/**
 * Generate a rich HTML message payload.
 * @param {string}      html
 * @param {object|null} quoted
 * @param {object}      options - { id?, title?, source?, trusted_sources?, typename?, headerText?, footer?, botJid?, mentions? }
 */
const generateRichHtmlContent = (html, quoted, options = {}) => {
    const crypto = require('crypto')
    if (quoted && !quoted.key && !quoted.message && typeof quoted === 'object') {
        options = quoted
        quoted = undefined
    ...
    const responseId = id ? `${id}-${Date.now()}` : crypto.randomUUID()
    const trusted = trusted_sources
        ? (Array.isArray(trusted_sources) ? trusted_sources : [trusted_sources])
        : (source ? [source] : [])
    const primitiveTypename = typename || 'GenAIaeacdsnwHtmlPrimitive'
    ...
    const ctxInfo = buildRichContextInfo(quoted, options)
    const botMsg = buildBotForwardedMessage(submessages, ctxInfo, unifiedResponse)
...
/**
 * Sends a rich HTML message using Baileys.
 * 
 * @param {Object} socket - The active Baileys socket instance used for communication.
 * @param {string} jid - The target JID.
 * @param {Object} options - Configuration options for the HTML message { id?, title?, html, source? }.
 * @param {Object} [quoted] - Optional quoted message.
 * @param {Object} [relayOptions] - Optional relayMessage options.
 */
async function sendRichHtml(socket, jid, { id, title, html, source, trusted_sources, typename } = {}, quoted, relayOptions = {}) {
    const { message, messageId } = generateRichHtmlContent(html, quoted, { id, title, source, trusted_sources, typename })
