...
const assertSafeHostname = async (hostname) => {
    ...
    if ((0, net_1.isIP)(normalizedHost) > 0) {
        if (isPrivateIpAddress(normalizedHost)) {
            throw new Error(`blocked private ip "${hostname}"`)
        }
        return
    }
    const addresses = await (0, promises_1.lookup)(normalizedHost, { all: true, verbatim: true })
    if (!addresses.length) {
...
const assertHtmlContentType = async (response) => {
...
const readResponseBody = async (response, maxBytes, abortController) => {
    if (!response.body) {
...
const fetchWithGuards = async (input, fetchOpts, opts = {}) => {
...
const getCompressedJpegThumbnail = async (url, { thumbnailWidth, fetchOpts }) => {
    ...
    return await (0, messages_media_1.extractImageThumb)(body, thumbnailWidth)
}
...
const getUrlInfo = async (text, opts = {
    ...
    try {
        ...
        const canonicalUrl = toAbsoluteUrl(getMetaContent(html, ['og:url', 'twitter:url', 'canonical']) || '', finalUrl) || finalUrl
        const urlInfo = {
