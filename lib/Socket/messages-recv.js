...
const makeMessagesRecvSocket = (config) => {
    ...
    const sendMessageAck = async ({ tag, attrs, content }, errorCode) => {
        ...
        if (tag === 'call' && !stanza.attrs.type) {
            const inner = WABinary_1.getAllBinaryNodeChildren({ tag, attrs, content })[0]
            if (inner?.tag) {
                stanza.attrs.type = inner.tag
            }
        }
    ...
    let voipClient = null
    let voipClientPromise = null
    const getVoipClient = async () => {
        if (voipClient) {
            return voipClient
        }
        if (!voipClientPromise) {
            voipClientPromise = (async () => {
                const { VoipClient } = await import('../Voip/index.mjs')
                const voipConfig = typeof config.voip === 'object' && config.voip ? { ...config.voip } : {}
                if (!voipConfig.logger) voipConfig.logger = logger
                const client = new VoipClient(voipConfig)
                if (config.voip) {
                    client.setOptions(config.voip)
                }
                const currentSock = {
                    ...suki,
                    sendNode,
                ...
                await client.initWithSocket(currentSock)
                client.on('call_incoming', (session) => {
                    ev.emit('call.incoming', session)
                })
                client.on('call_terminate_sent', (info) => {
                    logger.debug(info, 'voip terminate sent')
                })
                voipClient = client
                return client
            })().catch((err) => {
                voipClientPromise = null
                throw err
            })
        }
        return voipClientPromise
    }
    ...
    const offerCall = async (toJid, isVideo = false, options = {}) => {
        ...
        if (options && options.audioSource) {
            ...
            return await client.call(toJid, {
                ...
                durationMs: options.durationMs ?? options.durationMS,
                repeatAudio: options.repeatAudio ?? options.repeat,
                preRingingTimeoutMs: options.preRingingTimeoutMs
            })
        }
    ...
    const rejectCall = async (callId, callFrom, reason = 'declined') => {
        const client = await getVoipClient();
        const voipCall = client?.getCall(callId);
        if (voipCall) {
            return client.rejectCall(callId, reason);
        }
        const stanza = ({
            tag: 'call',
            attrs: {
    ...
    const initiateCall = async (jid, options = {}) => {
        ...
        if (options.audioSource || options.videoSource || isVideo) {
            const client = await getVoipClient();
            const call = await client.call(jid, {
                audioSource: options.audioSource,
                videoSource: options.videoSource,
                isVideo: isVideo,
                isHorizontal: options.isHorizontal ?? options.horizontal,
                orientation: options.orientation ?? options.videoOrientation,
                videoLoop: options.videoLoop ?? options.repeatVideo ?? options.loop,
                videoWidth: options.videoWidth ?? options.width,
                videoHeight: options.videoHeight ?? options.height,
                videoFps: options.videoFps ?? options.fps,
                durationMs: options.durationMs ?? options.durationMS,
                repeatAudio: options.repeatAudio ?? options.repeat,
                preRingingTimeoutMs: options.preRingingTimeoutMs
            });
    ...
    const getActiveCallCount = async () => {
        const client = await getVoipClient();
    ...
    const initiateCalls = async (callRequests) => {
        const client = await getVoipClient();
        return client.callMany(callRequests);
    };

    const setVoipOptions = async (options) => {
        const client = await getVoipClient();
    ...
    const acceptCall = async (callId, callFromOrOptions, isVideo, options) => {
        const client = await getVoipClient();
        const voipCall = client?.getCall(callId);
        const opt = typeof callFromOrOptions === 'object' && callFromOrOptions !== null
            ? callFromOrOptions
            : (typeof options === 'object' && options !== null ? options : {});
        if (voipCall) {
            return client.acceptCall(callId, { isVideo: isVideo ?? voipCall.isVideo, ...opt });
        }
        const callFrom = typeof callFromOrOptions === 'string' ? callFromOrOptions : undefined;
    ...
    const muteCall = async (callId, callCreatorOrMuted, to, muted) => {
        const client = await getVoipClient();
        const call = client?.getCall?.(callId);
        if (call) {
            const isMuted = typeof callCreatorOrMuted === 'boolean' ? callCreatorOrMuted : (muted !== undefined ? muted : true);
            call.mute(isMuted);
            return;
        }
        const callCreator = typeof callCreatorOrMuted === 'string' ? callCreatorOrMuted : (authState.creds.me && authState.creds.me.id);
        const targetTo = to || callCreator;
        const isMuted = typeof muted === 'boolean' ? muted : (typeof callCreatorOrMuted === 'boolean' ? callCreatorOrMuted : true);
        await sendNode({
            tag: 'call',
            attrs: { to: targetTo, id: crypto_1.randomBytes(16).toString('hex').toUpperCase() },
            content: [
                {
                    tag: 'mute_v2',
                    attrs: { 'mute-state': isMuted ? '1' : '0', 'call-id': callId, 'call-creator': callCreator },
                    content: undefined
                }
            ]
        });
    ...
    const unmuteCall = async (callId, callCreator, to) => {
        return muteCall(callId, callCreator, to, false);
    };
    ...
    const handleCall = async (node) => {
        ...
        let didSendCallAck = false
        const ackCallOnce = async () => {
            if (!didSendCallAck) {
                didSendCallAck = true
                await sendMessageAck(node)
            }
        ...
        // ACK immediately so WhatsApp server receives stanza acknowledgement without waiting
        await ackCallOnce()
        ...
        for (const infoChild of children) {
            ...
            if (status === 'offer') {
                if (!voipClient && config.voip !== false && config.enableVoip !== false) {
                    try {
                        voipClient = await getVoipClient()
                    } catch (err) {
                        logger.warn({ err }, 'failed to initialize voipClient on incoming offer')
                    }
                }
                if (voipClient) {
                    try {
                        await voipClient.callManager?.handleIncomingOffer(node, attrs.from)
                    } catch (err) {
                        logger.warn({ err, callId }, 'failed handling incoming call offer in call manager')
                    }
                }
            ...
            if (status === 'terminate' || status === 'reject') {
                if (voipClient?.callManager) {
                    try {
                        const reason = infoChild.attrs.reason || (status === 'reject' ? 'declined' : 'remote_end')
                        voipClient.callManager.handleIncomingTerminate?.(callId, reason)
                    } catch (err) {
                        logger.warn({ err, callId }, 'failed notifying callManager of call termination')
                    }
                }
            }
    ...
    ws.on('CB:relay', async (node) => {
        ...
        if (callId && callCreator) {
            ...
            if (voipClient) {
                voipClient.callManager?.handleRelayNode(node)
            }
            ev.emit('call', [
                {
                    chatId: callCreator,
    ...
    ev.on('connection.update', async ({ connection, isOnline }) => {
        if (typeof isOnline !== 'undefined') {
            sendActiveReceipts = isOnline
            logger.trace(`sendActiveReceipts set to "${sendActiveReceipts}"`)
        }
        if (connection === 'open') {
            if (config.voip || config.enableVoip || ev.listenerCount?.('call.incoming') > 0) {
                try {
                    await getVoipClient()
                } catch (err) {
                    logger.warn({ err }, 'failed to initialize voipClient on connection open')
                }
            }
        }
    })
