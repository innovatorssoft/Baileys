...
const makeUsernameSocket = (config) => {
    ...
    const resolveUsername = async (username) => {
        ...
        try {
            const q = new USyncQuery().withContext('interactive').withMode('query').withUsernameProtocol().withLIDProtocol().withContactProtocol().withUser(new USyncUser().withUsername(normalized))
