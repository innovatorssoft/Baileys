...
const { isLidUser, isJidUser, jidNormalizedUser } = require("../WABinary")
...
const makeUsernameSocket = (config) => {
    ...
    const toResolution = (username, entry, extraPn) => {
        const lid = entry?.lid || (entry?.id && isLidUser(entry.id) ? entry.id : undefined)
        const rawPn = entry?.pn || extraPn || (entry?.id && isJidUser(entry.id) ? entry.id : undefined)
        const pn = rawPn ? jidNormalizedUser(rawPn) : undefined
        return { username, jid: pn || lid || entry?.id, ...(lid ? { lid } : {}), ...(pn ? { pn } : {}) }
    }
    const resolvePNForLID = async (lid) => {
        if (!lid || !isLidUser(lid)) return undefined
        if (signalRepository?.lidMapping?.getPNForLID) {
            try {
                const mappedPn = await signalRepository.lidMapping.getPNForLID(lid)
                if (mappedPn && isJidUser(mappedPn)) return jidNormalizedUser(mappedPn)
            } catch (e) {
                logger?.debug?.({ lid, err: e }, '[Username] Error fetching PN from lidMapping')
            }
        }
        if (typeof sock.onWhatsApp === 'function') {
            try {
                const waList = await sock.onWhatsApp(lid)
                const wa = waList?.[0]
                const foundPn = (wa?.jid && isJidUser(wa.jid)) ? wa.jid : ((wa?.pn && isJidUser(wa.pn)) ? wa.pn : undefined)
                if (foundPn) {
                    const normPn = jidNormalizedUser(foundPn)
                    if (signalRepository?.lidMapping?.storeLIDPNMappings) {
                        await signalRepository.lidMapping.storeLIDPNMappings([{ pn: normPn, lid }])
                    }
                    return normPn
                }
            } catch (e) {
                logger?.debug?.({ lid, err: e }, '[Username] Error fetching PN via onWhatsApp')
            }
        }
        try {
            const lidQuery = new USyncQuery().withContactProtocol().withLIDProtocol().withUser(new USyncUser().withId(lid).withLid(lid))
            const lidEntry = (await executeUSyncQuery(lidQuery))?.list?.[0]
            const foundPn = lidEntry?.pn || (lidEntry?.id && isJidUser(lidEntry.id) ? lidEntry.id : undefined)
            if (foundPn) {
                const normPn = jidNormalizedUser(foundPn)
                if (signalRepository?.lidMapping?.storeLIDPNMappings) {
                    await signalRepository.lidMapping.storeLIDPNMappings([{ pn: normPn, lid }])
            ...
            logger?.debug?.({ lid, err: e }, '[Username] Error fetching PN via USync LID query')
        }
        return undefined
    }
    const resolveUsername = async (username) => {
        ...
        try {
            ...
            let lid = entry.lid || (entry.id && isLidUser(entry.id) ? entry.id : undefined)
            let pn = entry.pn || (entry.id && isJidUser(entry.id) ? entry.id : undefined)
            if (lid && !pn) {
                pn = await resolvePNForLID(lid)
            }
            const resolution = toResolution(normalized, entry, pn)
    ...
    const resolveUsernames = async (usernames) => {
        ...
        try {
            ...
            for (const name of names) {
                ...
                const resolution = toResolution(name, entry, pn); await setCache(`user:${name}`, { ...resolution, resolvedAt: Date.now() }, DEFAULT_CACHE_TTLS.USERNAME); for (const index of indices) results[index] = resolution
