...
const makeGroupsSocket = (config) => {
    ...
    const groupMetadata = async (jid) => {
        ...
        if (suki.signalRepository?.lidMapping?.storeLIDPNMappings) {
            const mappings = []
            for (const p of metadata.participants) {
                if (p.lid && p.phoneNumber) {
                    mappings.push({ lid: p.lid, pn: p.phoneNumber })
                }
            }
            if (mappings.length > 0) {
                try {
                    await suki.signalRepository.lidMapping.storeLIDPNMappings(mappings)
                } catch (_) {}
            }
        }
        return metadata
...
const extractGroupMetadata = (result) => {
    ...
    const metadata = {
        ...
        participants: WABinary_1.getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            const isLidMode = mode === Types_1.WAMessageAddressingMode.LID
            const jidIsLid = WABinary_1.isLidUser(attrs.jid)
            const jidIsPn = WABinary_1.isPnUser(attrs.jid)
            const pnIsPn = WABinary_1.isPnUser(attrs.phone_number)
            const lidIsLid = WABinary_1.isLidUser(attrs.lid)

            const lid = jidIsLid ? attrs.jid : (lidIsLid ? attrs.lid : (isLidMode && attrs.jid && !jidIsPn ? attrs.jid : undefined))
            const phoneNumber = pnIsPn ? attrs.phone_number : (jidIsPn ? attrs.jid : undefined)
            const rawId = isLidMode ? (attrs.phone_number || attrs.jid) : (attrs.jid || attrs.phone_number)

            let username = attrs.username
            if (!username && rawId && (rawId.startsWith('@') || (!rawId.includes('@') && !/^\d+$/.test(rawId)))) {
                username = rawId.replace(/^@/, '')
            }
