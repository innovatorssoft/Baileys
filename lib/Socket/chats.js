...
const makeChatsSocket = (config) => {
    ...
    */
    ...
    const fetchBroadcastListQuota = async () => {
        ...
        const limitsNode = WABinary_1.getBinaryNodeChild(result, 'limits')
        const timeframeNode = WABinary_1.getBinaryNodeChild(result, 'timeframe')
        if (!limitsNode) return null
        return {
            messagesLeft: parseInt(limitsNode.attrs?.messages_left ?? WABinary_1.getBinaryNodeChild(limitsNode, 'messages_left')?.content ?? '0', 10),
            totalLimit: parseInt(limitsNode.attrs?.total_limit ?? WABinary_1.getBinaryNodeChild(limitsNode, 'total_limit')?.content ?? '0', 10),
            isHeavySender: (limitsNode.attrs?.is_heavy_sender ?? WABinary_1.getBinaryNodeChild(limitsNode, 'is_heavy_sender')?.content) === 'true',
            startTs: parseInt(timeframeNode?.attrs?.start_ts_s ?? WABinary_1.getBinaryNodeChild(timeframeNode, 'start_ts_s')?.content ?? '0', 10),
            endTs: parseInt(timeframeNode?.attrs?.end_ts_s ?? WABinary_1.getBinaryNodeChild(timeframeNode, 'end_ts_s')?.content ?? '0', 10),
            resetTs: parseInt(timeframeNode?.attrs?.reset_ts_s ?? WABinary_1.getBinaryNodeChild(timeframeNode, 'reset_ts_s')?.content ?? '0', 10)
        }
    }
    ...
    const getChatBlockingStatus = async () => {
        ...
        const blocking = WABinary_1.getBinaryNodeChild(result, 'blocking') ||
            WABinary_1.getBinaryNodeChild(WABinary_1.getBinaryNodeChild(result, 'query'), 'blocking')
    ...
    const getUserDisclosures = async (t = 0) => {
        ...
        const container = WABinary_1.getBinaryNodeChild(result, 'get_user_disclosures') ||
                          WABinary_1.getBinaryNodeChild(result, 'disclosures') ||
                          result
        return WABinary_1.getBinaryNodeChildren(container, 'notice').map(n => ({ ...n.attrs }))
    }
    ...
    const getOptOutList = async () => {
        const result = await query({
            tag: 'iq',
            attrs: { to: WABinary_1.S_WHATSAPP_NET, xmlns: 'privacy:optout:list', type: 'get' }
        })
        const listNode = WABinary_1.getBinaryNodeChild(result, 'optout_list')
        return WABinary_1.getBinaryNodeChildren(listNode, 'item').map(item => ({ ...item.attrs }))
    }

    const signPrivateCredential = async (identity) => {
        const result = await query({
            tag: 'iq',
            attrs: { to: WABinary_1.S_WHATSAPP_NET, xmlns: 'privacy:credit', type: 'set' },
            content: [{
                tag: 'sign_credential',
                attrs: {},
        ...
        const signedNode = WABinary_1.getBinaryNodeChild(result, 'signed_credential')
        return signedNode?.content instanceof Uint8Array ? Buffer.from(signedNode.content) : undefined
    }
    ...
    const getPushConfig = async () => {
        const result = await query({
            ...
            content: [{ tag: 'config', attrs: {} }]
        })
        const configNode = WABinary_1.getBinaryNodeChild(result, 'config')
        return configNode
    }
    ...
    const toggleCallLinkWaitingRoom = async (token, enable) => {
        await query({
            tag: 'iq',
            attrs: { to: WABinary_1.S_WHATSAPP_NET, xmlns: 'w:voip:call_link', type: 'set' },
            content: [{
                tag: 'waiting_room',
                attrs: { token: String(token), action: enable ? 'enable' : 'disable' }
            }]
        })
    }
    ...
    const updateBioPrivacy = async (value) => {
        await query({
            ...
            content: [{
                ...
                content: [{
                    tag: 'category',
                    attrs: { name: 'bio', value: String(value) }
                }]
            }]
        })
    }
    ...
    const blockBot = async (botJid) => {
        await updateBlockStatus(botJid, 'block')
    }
    ...
    const getBotProfile = async (botJid) => {
        try {
            const usyncQuery = new WAUSync_1.USyncQuery()
                ...
                .withUser(new WAUSync_1.USyncUser().withId(botJid))
            const result = await suki.executeUSyncQuery(usyncQuery)
            const botData = result?.list?.[0]?.bot
    ...
    const fetchABProps = async (protocol = '1', hash = '', refreshId = null, group = null) => {
        ...
        const propsNode = WABinary_1.getBinaryNodeChild(result, 'props')
        if (!propsNode) return {}
    ...
    const sendKeyIndexList = async () => {
        ...
        const accountType = deviceIdentity.accountType ?? WAProto_1.proto.ADVEncryptionType.E2EE
        const isHosted = accountType === WAProto_1.proto.ADVEncryptionType.HOSTED
        const ts = Math.floor(Date.now() / 1000)
        const details = WAProto_1.proto.ADVKeyIndexList.encode({
        ...
        const sigPrefix = isHosted ? Defaults_1.WA_ADV_HOSTED_KEY_INDEX_LIST_SIG_PREFIX : Defaults_1.WA_ADV_KEY_INDEX_LIST_SIG_PREFIX
        ...
        const content = WAProto_1.proto.ADVSignedKeyIndexList.encode(signedKeyIndexList).finish()
    ...
    const fetchMediaConn = async (lastId = null) => {
        ...
        return WABinary_1.getBinaryNodeChild(result, 'media_conn') || null
    }
    ...
    const fetchQRCode = async (code, addressingMode = null) => {
        ...
        return WABinary_1.getBinaryNodeChild(result, 'qr') || null
    }
    ...
    const findUserId = async (pnLid) => {
        const normalizedJid = (0, WABinary_1.jidNormalizedUser)(pnLid)
        const userId = {
            lid: undefined,
            phoneNumber: undefined
        }
        if ((0, WABinary_1.isPnUser)(normalizedJid) || (0, WABinary_1.isHostedPnUser)(normalizedJid)) {
            userId.phoneNumber = normalizedJid
            const resolvedLid = (await signalRepository.lidMapping?.getLIDsForPNs?.([normalizedJid]))?.[0]?.lid
            userId.lid = resolvedLid ? (0, WABinary_1.jidNormalizedUser)(resolvedLid) : undefined
        } else if ((0, WABinary_1.isLidUser)(normalizedJid) || (0, WABinary_1.isHostedLidUser)(normalizedJid)) {
            userId.lid = normalizedJid
            const resolvedPhoneNumber = (await signalRepository.lidMapping?.getPNsForLIDs?.([normalizedJid]))?.[0]?.pn
            userId.phoneNumber = resolvedPhoneNumber ? (0, WABinary_1.jidNormalizedUser)(resolvedPhoneNumber) : undefined
        } else {
