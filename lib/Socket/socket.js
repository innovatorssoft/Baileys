...
const makeSocket = (config) => {
    ...
    const executeUSyncQuery = async (usyncQuery) => {
        ...
        const result = await (sock?.query || query)(iq)
        return usyncQuery.parseUSyncQueryResult(result)
    }

    const onWhatsApp = async (...jids) => {
        const executeQuery = (...args) => (sock?.executeUSyncQuery || executeUSyncQuery)(...args)
        let usyncQuery = new WAUSync_1.USyncQuery().withLIDProtocol()
        const lidQuery = new WAUSync_1.USyncQuery().withContext('interactive').withDeviceProtocol()
        const usernameQuery = new WAUSync_1.USyncQuery().withContactProtocol()

        const usernames = []
        let contactEnabled = false
        const items = jids.flat()
        ...
        for (const item of items) {
            ...
            if (typeof target === 'object' && target !== null) {
                ...
                } else if (target.username && typeof target.username === 'string') {
                    isUsername = true
                    target = target.username
                } else if (target.jid && typeof target.jid === 'string') {
                    target = target.jid
                } else if (target.lid && typeof target.lid === 'string') {
                    target = target.lid
                }
            }
            ...
            if (typeof target === 'string') {
                ...
                if (isUsername || trimmed.startsWith('@') || (Utils_1.isValidUsername?.(trimmed) && !trimmed.includes('@'))) {
                    try {
                        const uname = Utils_1.normalizeUsername?.(trimmed) || trimmed.replace(/^@/, '').toLowerCase()
                        usernames.push(uname)
                        usernameQuery.withUser(new WAUSync_1.USyncUser().withUsername(uname))
                    } catch { }
                } else if (WABinary_1.isLidUser(trimmed)) {
                    lidQuery.withUser(new WAUSync_1.USyncUser().withId(trimmed).withLid(trimmed))
                } else {
                    if (!contactEnabled) {
                        contactEnabled = true
                        usyncQuery.withContactProtocol()
                    }
                    const phone = `+${trimmed.replace('+', '').split('@')[0]?.split(':')[0]}`
        ...
        if (usyncQuery.users.length > 0) {
            try {
                const results = await executeQuery(usyncQuery)
                if (results) {
                    ...
                    const mapped = results.list
                        .filter(a => !!a.contact)
                        .map(({ contact, id, lid, pn }) => ({ jid: id, exists: !!contact, ...(lid ? { lid } : {}), ...(pn ? { pn } : {}) }))
                    output.push(...mapped)
                }
            } catch (err) {
                logger?.error?.({ err }, 'Error querying phone presence via USync')
            }
        }

        if (lidQuery.users.length > 0) {
            try {
                const lidResults = await executeQuery(lidQuery)
                if (lidResults?.list) {
                    const mappings = []
                    for (const entry of lidResults.list) {
                        if (!entry || entry.error) continue
                        const lid = entry.lid || (entry.id && WABinary_1.isLidUser(entry.id) ? entry.id : undefined)
                        if (!lid) continue
                        let pn = entry.pn || (entry.id && WABinary_1.isJidUser(entry.id) ? entry.id : undefined)
                        if (!pn && signalRepository?.lidMapping?.getPNForLID) {
                            try {
                                const mappedPn = await signalRepository.lidMapping.getPNForLID(lid)
                                if (mappedPn && WABinary_1.isJidUser(mappedPn)) {
                                    pn = WABinary_1.jidNormalizedUser(mappedPn)
                                }
                            } catch { }
                        }
                        if (lid && pn) {
                            mappings.push({ pn, lid })
                        }
                        output.push({
                            jid: pn || lid,
                            exists: true,
                    ...
                    if (mappings.length > 0 && signalRepository?.lidMapping?.storeLIDPNMappings) {
                        await signalRepository.lidMapping.storeLIDPNMappings(mappings)
                    }
                }
            } catch (err) {
                logger?.error?.({ err }, 'Error querying LID presence via USync')
            }
        }

        if (usernames.length > 0) {
            try {
                const usernameResults = await executeQuery(usernameQuery)
                if (usernameResults?.list) {
                    const mappings = []
                    for (let i = 0; i < usernameResults.list.length; i++) {
                        const entry = usernameResults.list[i]
                        const uname = usernames[i] || entry?.username
                        if (!entry || entry.error || (!entry.id && !entry.lid && !entry.pn)) {
                            continue
                        }
                        let lid = entry.lid || (entry.id && WABinary_1.isLidUser(entry.id) ? entry.id : undefined)
                        let pn = entry.pn || (entry.id && WABinary_1.isJidUser(entry.id) ? entry.id : undefined)

                        if (lid && !pn) {
                            if (signalRepository?.lidMapping?.getPNForLID) {
                                try {
                                    const mappedPn = await signalRepository.lidMapping.getPNForLID(lid)
                                    if (mappedPn && WABinary_1.isJidUser(mappedPn)) {
                                        pn = WABinary_1.jidNormalizedUser(mappedPn)
                            ...
                            if (!pn) {
                                try {
                                    const qLid = new WAUSync_1.USyncQuery().withContactProtocol().withLIDProtocol().withUser(new WAUSync_1.USyncUser().withId(lid).withLid(lid))
                                    const lidRes = (await executeQuery(qLid))?.list?.[0]
                                    const foundPn = lidRes?.pn || (lidRes?.id && WABinary_1.isJidUser(lidRes.id) ? lidRes.id : undefined)
                                    if (foundPn && WABinary_1.isJidUser(foundPn)) {
                                        pn = WABinary_1.jidNormalizedUser(foundPn)
                                    }
                                } catch { }
                            }
                        }
                        ...
                        output.push({
                            jid: pn || lid || entry.id,
                            exists: !entry.error && (!!entry.contact || !!entry.id || !!lid),
                            ...(lid ? { lid } : {}),
                            ...(pn ? { pn } : {}),
                ...
                logger?.error?.({ err }, 'Error querying username presence via USync')
            }
        }
